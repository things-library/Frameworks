        ...
        public static IServiceCollection AddDatabaseSqlServer<TContext>(this IServiceCollection services, string connectionString) where TContext : Database.DataContext
        {
            ...
            services.AddDbContext<TContext>(options =>
            {
                ...
                options.UseSqlServer(connectionString, contextOptions => {
                    contextOptions.MigrationsAssembly(typeof(TContext).Assembly.FullName);
                    contextOptions.CommandTimeout((int)TimeSpan.FromSeconds(30).TotalSeconds);

                    contextOptions.EnableRetryOnFailure(maxRetryCount: 15, maxRetryDelay: TimeSpan.FromSeconds(30), errorNumbersToAdd: null);
                });
            });
