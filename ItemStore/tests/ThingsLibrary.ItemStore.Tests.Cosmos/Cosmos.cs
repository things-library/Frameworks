// ================================================================================
// <copyright file="Cosmos.cs" company="Starlight Software Co">
...
namespace ThingsLibrary.ItemStore.Tests.Cosmos
{
    ...
    public class CosmosDB : UnitTests
    {
        ...
        public static void ClassInit(TestContext context)
        {
            try
            {
                ...
                var resourceKey = "databases/cosmos";
                ...
                ItemStore = new ItemStoreCosmos(ConnectionString, DatabaseName, CollectionName);
            }
            catch(ArgumentException ex)
            {
                Log.Error(ex, "CosmosDB ClassInit failed");
            }
        }
        ...
        public static void ClassCleanup()
        {
            try
            {
                if (ItemStore != null)
                {
                    var cosmosEntityStore = (ItemStoreCosmos)ItemStore;

                    // Delete the container
                    cosmosEntityStore.Container.DeleteContainerAsync().Wait();
                }
            }
            catch(Exception ex)
            {
                Log.Error(ex, "CosmosDB ClassCleanup failed");
            }
        ...
        {   
            var store = new ItemStoreCosmos(ConnectionString, DatabaseName, CollectionName);

            Assert.AreEqual(ItemStoreType.Azure_Cosmos, store.StoreType);
