...
namespace Starlight.Entity.Tests.Integration.LocalMemory
{
    [TestClassIf, IgnoreIf(nameof(IgnoreTests)), ExcludeFromCodeCoverage]
    public class LocalMemoryTests //: Base.IBaseTests
    {        
        ...
        {
            var entityStore = new Local.EntityStore<TestClass>("Filename=:memory:", TableName);            
            Assert.IsTrue(entityStore.IsMemoryDb);
            Assert.IsFalse(entityStore.IsFileSaving);
