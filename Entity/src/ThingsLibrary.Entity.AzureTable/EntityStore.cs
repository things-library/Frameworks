...
namespace ThingsLibrary.Entity.AzureTable
{
    ...
    public class EntityStore<T> : IEntityStore<T> where T : class
    {
        ...
        /// <inheritdoc />
        public T GetEntity(object key, object partitionKey, T defaultValue)
        {
        ...
        {
            NullableResponse<Az.TableEntity> response;
            if (this.PartitionKey == null)
            {
                response = await this.AzureCloudTable.GetEntityIfExistsAsync<Az.TableEntity>(string.Empty, key.ToString(), null, cancellationToken);
            }
            else
            {
                response = await this.AzureCloudTable.GetEntityIfExistsAsync<Az.TableEntity>(partitionKey.ToString(), key.ToString(), null, cancellationToken);
            }

            if (!response.HasValue) { return defaultValue;  }

            return this.ToEntity(response.Value);
        }
